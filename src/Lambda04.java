...
public class Lambda04 {
	...
	public static void main(String[] args) {
		...
		System.out.println(fazla130danEnBuyuk(list));
		
		System.out.println(az150MinBatch(list));
	...
	// task 02-->ogrc sayilarinin hic birinin 110 den az olmadigini kontrol eden pr
	// create ediniz.
	...
	public static List<Integer> ogrSayisiSiralaBK(List<TechPro> list) {
		...
		List<Integer> sayilar = list.stream().map(t -> t.getOgrcSayisi()).sorted(Comparator.reverseOrder())
				.collect(Collectors.toList());
		return sayilar;
	}

	// task 05 --> Batch'leri batch ortalamasina göre b->k siralayip ilk 3 ünü
	// yazdırınız.

	public static List<TechPro> ortalamaSiralamasi(List<TechPro> list) {

		return list.stream().sorted(Comparator.comparing(TechPro::getBatchOrtalama).reversed()).limit(3)
				.collect(Collectors.toList());
	...
	public static List<TechPro> ogrSayisiEnAz2Batch(List<TechPro> list) {
		...
		return list.stream().sorted(Comparator.comparing(TechPro::getOgrcSayisi)).limit(2).skip(1)
				.collect(Collectors.toList());

	}

	// task 07 --> batch ort 95 'den buyuk olan batch'lerin ogrc sayilarini
	...
	public static OptionalDouble ogrcSayisi130BykBatchOrt(List<TechPro> list) {
		return list.stream().filter(t -> t.getOgrcSayisi() > 130).mapToDouble(t -> t.getBatchOrtalama()).average();
	...
	public static OptionalInt fazla130danEnBuyuk(List<TechPro> list){
		  return  list.
		            stream()
		            .filter(t->t.getOgrcSayisi()>130).mapToInt(TechPro::getBatchOrtalama).max();

		}
	
	// task 11-->Ogrenci sayilari 150'dan az olan batch'lerin en kucuk batch ort'unu bulunuz.
	
	public static OptionalInt az150MinBatch(List<TechPro> list){
		  return  list.
		            stream()
		            .filter(t->t.getOgrcSayisi()<150).mapToInt(TechPro::getBatchOrtalama).min();
